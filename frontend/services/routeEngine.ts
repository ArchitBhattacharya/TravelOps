// ============================================================
// TravelOps — Dynamic Route Engine
// Generates realistic routes based on actual crisis inputs.
// Used by both demo mode (client) and the scoring layer (server).
// ============================================================

import type { TravelCrisis, TravelRoute, RouteSegment, RiskLevel, RouteStatus } from '@/types/travel';
...
export function scoreRoute(
  arrivalMins: number,     // minutes from now
  deadlineMins: number,    // minutes from now
  price: number,
  budget: number,
  transfers: number,
  ...
  const budgetRatio = Math.max(0, 1 - price / budget);           // 0–1, higher = cheaper
  const bufferRatio = Math.min(1, (deadlineMins - arrivalMins) / (4 * 60)); // 0–1
...
export function generateRoutes(crisis: TravelCrisis): GeneratedRoutes {
  ...
  // Seed from origin+destination string so results are stable per pair but differ across pairs
  ...
  if (!preferences.preferDirect) {
    ...
    const deadlineMet = arr <= deadlineDate;
    const overBudget = price > maxBudget;
    const hub        = pickNum(['Mumbai', 'Hyderabad', 'Chennai', 'Bengaluru', 'Pune'], seed + 6);
    ...
    routes.push({
      ...
      primaryMode: 'flight',
      segments: [
        { mode: 'flight', from: `${origin} Airport`, to: `${hub} Airport`, departure: formatTime(dep), arrival: formatTime(midArr), carrier: c1, duration: formatDuration(leg1Mins), transferTime: `${layover}m layover` },
        { mode: 'flight', from: `${hub} Airport`, to: `${destination} Airport`, departure: formatTime(midDep), arrival: formatTime(arr), carrier: c2, duration: formatDuration(leg2Mins) },
      ],
      totalPrice: price,
      currency,
      finalArrival: formatTime(arr),
      ...
      rejectionReason: overBudget
        ...
        : riskLevel !== 'LOW' ? `Only ${layover}m layover in ${hub} — high connection risk.` : undefined,
    });
  }
  ...
  // ── Option D: Bus (only if not avoided) ──────────────────
  if (!preferences.avoidBus) {
    ...
    routes.push({
      ...
      segments: [{
        mode: 'bus',
        from: `${origin} Bus Stand`,
        to: `${destination} Bus Stand`,
        departure: formatTime(dep),
        arrival: formatTime(arr),
        carrier,
  ...
  // Sort: recommended first, then viable, then rejected; within each group by score desc
  ...
  const totalFound = routes.length + Math.round(seed % 8) + 5; // simulate more found than shown

  return { routes, totalFound, eliminated, viable, recommended };
}
